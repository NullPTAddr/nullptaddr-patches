package app.morphe.extension.dudu.patches.spoof
...
class SpoofSignature {
    companion object {
        ...
        fun spoofSignature(_packageInfo: PackageInfo, _signature: Signature): PackageInfo {
            val signatures = _packageInfo.signatures
            if (signatures != null && signatures.size > 0) {
                signatures[0] = _signature
            }
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                val signingInfo = _packageInfo.signingInfo
                if (signingInfo != null) {
        ...
        fun addHiddenApiExemptions() {
            try {
                ...
                val getRuntime = vmRuntimeClass.getDeclaredMethod(
                    "getRuntime"
                )
